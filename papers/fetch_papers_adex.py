...
text_search = "ti:adversarial OR ti:attack OR ti:defense OR ti:robustness OR ti:security"
query = f"(cat:cs.CR OR cat:cs.AI OR cat:cs.CV OR cat:cs.LG) AND (({text_search}) OR (au:carlini AND ({text_search})))"
...
# Save to JSON file
with open(os.path.join('assets/json', 'adex_papers.json'), 'w') as f:
